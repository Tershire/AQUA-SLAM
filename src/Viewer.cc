...
Viewer::Viewer(System* pSystem, FrameDrawer *pFrameDrawer, Tracking *pTracking, const string &strSettingPath):
    both(false), mpSystem(pSystem), mpFrameDrawer(pFrameDrawer), mpTracker(pTracking),
...
//    cv::namedWindow("ORB-SLAM3: Current Frame");
    ...
    if(mpTracker->mSensor == mpSystem->MONOCULAR || mpTracker->mSensor == mpSystem->STEREO||mpTracker->mSensor == mpSystem->DVL_STEREO || mpTracker->mSensor == mpSystem->RGBD)
    {
        menuShowGraph = true;
    }
    ...
    while(1)
    {
        ...
        if(menuShowPoints)
			...
			mpMapDrawer->DrawMapPointsWithColor();
