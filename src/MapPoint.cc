...
namespace ORB_SLAM3
{
    ...
    // idx: index of the map point in the keyframe
    void MapPoint::AddObservation(KeyFrame *pKF, int idx)
    {
        unique_lock<mutex> lock(mMutexFeatures);
        tuple<int, int> indexes;

        // if this map point has been seen in this key frame
        ...
        /* NLeft/Nleft: the number of ORB feature key points extracted in the left image
        ** when using monocular, first int: map point index
        ** when using stereo, first int: left index, second int: right index, first left then right
        */
        // NLeft/Nleft=-1 means monocular only
