    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(NewDetectCommonRegions())
            {
                ...
                // current frame belong to different map(incative map)
                // do map merge
                if(mbMergeDetected)
                {
                    ...
                    else if(mTargetMapID!=-1&&mTargetMapID!=mpMergeMatchedKF->GetMap()->GetId())
					{
                    	cout<<"map merging is not in target map, merge is aborted"<<endl;
					}
                    else
                    {
						...
						cout<<"***************Merged detected**********************"<<endl;
                        // we are going to move all map points in inactive map into active map,
                        // mpMergeMatchedKF: // the best keyframe selected from the candidate ketframes

                        // mTmw: transforamtion from origin(inactive map) to map point(inactive map)
                        cv::Mat mTmw = mpMergeMatchedKF->GetPose();
                        // w2: inactive map origin
                        // gSmw2: simlilarity transformation from inactive map origin to map point(inactive map)
                        g2o::Sim3 gSmw2(Converter::toMatrix3d(mTmw.rowRange(0, 3).colRange(0, 3)),Converter::toVector3d(mTmw.rowRange(0, 3).col(3)),1.0);
                        // mTcw: transforamtion from origin(active map) to camera(active map)
                        cv::Mat mTcw = mpCurrentKF->GetPose();
                        // w1: active map origin
                        // gScw1: simlilarity transformation from active map origin to camera
                        g2o::Sim3 gScw1(Converter::toMatrix3d(mTcw.rowRange(0, 3).colRange(0, 3)),Converter::toVector3d(mTcw.rowRange(0, 3).col(3)),1.0);
                        // mg2oMergeSlw: simlilarity transformation from inactive map origin to camera
...
//							unique_lock<mutex> lock(mMutexReset);
                ...
                // current keyframe belong to the active map
                // do loop closure
                if(mbLoopDetected)
                {
                    vdPR_CurrentTime.push_back(mpCurrentKF->mTimeStamp);
...
//	if less than 3 keyframes in map, return false
    ...
    // Loop candidates
   bool bLoopDetectedInKF = false;//what is this variable mean?????????
    bool bCheckSpatial = false;

    // when mnLoopNumCoincidences is more then 0????????
    if(mnLoopNumCoincidences > 0)
    {
        bCheckSpatial = true;
        // Find from the last KF candidates
        cv::Mat mTcl = mpCurrentKF->GetPose() * mpLoopLastCurrentKF->GetPoseInverse();//T_cw*T_wl
    ...
    bool bMergeDetectedInKF = false;//what does this variable mean????????????
...
// vpBowCand: candidate ketframes may have data association with current ketframe
// pMatchedKF2: the best keyframe from the candidate ketframes
                                             ...
                                             int &nNumCoincidences, std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    for(KeyFrame* pKFi : vpBowCand)
    {
        ...
        /**
         * vpCovKFi: keyframes have good covisibility with the candidate keyframe(previous keyframe in the map)
         * and the candidate keyframe
         */
        std::vector<KeyFrame*> vpCovKFi = pKFi->GetBestCovisibilityKeyFrames(nNumCovisibles);
		if(vpCovKFi.empty())
			continue;//fix crash after map merging
        vpCovKFi.push_back(vpCovKFi[0]);
        vpCovKFi[0] = pKFi;

        // vvpMatchedMPs: all matched map points in vpCovKFi(keyframes have good covisibility with the candidate keyframe and the candidate keyframe)
        std::vector<std::vector<MapPoint*> > vvpMatchedMPs;
        vvpMatchedMPs.resize(vpCovKFi.size());

        // spMatchedMPi: matched map points in vvpMatchedMPs after removing some bad points
        std::set<MapPoint*> spMatchedMPi;
        //size of spMatchedMPi
        int numBoWMatches = 0;

        // current keyframe candidate(previous keyframes in the map) which may have loop
        ...
        /***
         * do feature matching between keyframes arround the candidate keyframe and the current keyframes
         */
        ...
        /***
         * iterate all matched map points in vvpMatchedMPs
         * and remove some bad points
         */
        ...
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
            ...
            {
				/**
				 * if this keyframe arround candidate keyframe is connected to the current keyframe
				 * then ignore this keyframe
				 */
                bAbortByNearKF = true;
       ...
       //ROS_INFO_STREAM("numBoWMatches:"<<numBoWMatches<<" threshold:"<<nBoWMatches);
        if(!bAbortByNearKF && numBoWMatches >= nBoWMatches) // TODO pick a good threshold
        {
            ...
            /**
             * set RANSAC Solver
             * according to the 3D aligning transformation part in the paper
             */
            ...
            if(bConverge)
            {
                ...
                // map points around the candidate frame
                vector<MapPoint*> vpMapPoints;
				/***
				 * pointer to keyframes around the candidate keyframe
				 * and has the same index with vpMapPoints
				 */
                ...
                /**
                 * gScm: the Transformation from keyframe candidate to current keyframe
                 * gScm obtained by the RANSAC Solver
                 */
...
//                cout <<"BoW: " << numProjMatches << " matches between " << vpMapPoints.size() << " points with coarse Sim3" << endl;
                ...
                if(numProjMatches >= nProjMatches)
                {
                    ...
                    if(numOptMatches >= nSim3Inliers)
                    {
                        ...
                        g2o::Sim3 gSmw(Converter::toMatrix3d(pMostBoWMatchesKF->GetRotation()),Converter::toVector3d(pMostBoWMatchesKF->GetTranslation()),1.0);
                        /**
                         * gScm: keyframe candidate to current keyframe(obtained by RANSAC Solver and Optimized by g2o)
                         * gSmw: world to keyframe candidate
                         * gScw: world to current keyframe(the loop result, not the odometry result!)
                         */
                        g2o::Sim3 gScw = gScm*gSmw; // Similarity matrix of current from the world position
                        cv::Mat mScw = Converter::toCvMat(gScw);
                         ...
                         * check whether the keyframe with good covisibility also match the thransformation
                         */
...
//                            cout << "BoW: There are " << numProjOptMatches << " matches between them with the optimized Sim3" << endl;
							...
							//the keyframes has good covisibility with current keyframe
                            vector<KeyFrame*> vpCurrentCovKFs = mpCurrentKF->GetBestCovisibilityKeyFrames(nNumCovisibles);

                            /**
                             * for each keyframes with good covisibility with current keyframe
                             * check whether they also match the transformation
                             * if match, nNumKFs++
                             * nNumKFs=nBestNumCoindicendes=nNumCoincidences
                             */
                            int j = 0;
...
//			Verbose::PrintMess("Other map KF, this should't happen", Verbose::VERBOSITY_DEBUG);
...

	{
		...
		for (MapPoint *pMPi: spLocalWindowMPs) {
			if (!pMPi || pMPi->isBad())
				continue;
			if (!(pMPi->GetMap()) || (pMPi->GetMap()->IsBad()) || (pMPi->GetMap() != pCurrentMap)) {
				pMPi->SetBadFlag();
				continue;
			}
		...
		std::chrono::duration<double, std::milli>
			timeTransfMerge = timeFinishTransfMerge - timeStartTransfMerge; // Time in milliseconds
...
//        unique_lock<timed_mutex> mergeLock(pMergeMap->mMutexMapUpdate); // We remove the Kfs and MPs in the merged area from the old map
		...
		mpAtlas->ChangeMap(pCurrentMap, pMergeMap);
		if (pCurrentMap->GetAllKeyFrames().size() == 0)
...
//        pMergeMap->ChangeId(pCurrentMap->GetId());
...
#endif
		std::chrono::duration<double, std::milli>
			timeCritMerge = timeFinishCritMerge - timeStartCritMerge; // Time in milliseconds
		...
		Verbose::PrintMess("MERGE-VISUAL: LOCAL MAPPING number of KFs: " + to_string(mpLocalMapper->KeyframesInQueue()),
		...
		pNewParent =
			mpCurrentKF; // Old child, now it will be the parent of its own parent(we need eliminate this KF from children list in its old parent)
		mpCurrentKF->ChangeParent(mpMergeMatchedKF);
		while (pNewChild /*&& spLocalWindowKFs.find(pNewChild) != spLocalWindowKFs.end()*/) {
			pNewChild
				->EraseChild(pNewParent); // We remove the relation between the old parent and the new for avoid loop
...
#endif
		std::chrono::duration<double, std::milli>
			timeFuseMerge = timeFinishFuseMerge - timeStartFuseMerge; // Time in milliseconds
		Verbose::PrintMess("MERGE-VISUAL: FUSE DUPLICATED ms: " + to_string(timeFuseMerge.count()),
		                   Verbose::VERBOSITY_DEBUG);
...
//		unique_lock<timed_mutex> currentLock(pMap->mMutexMapUpdate,std::defer_lock); // We update the current map with the Merge information
//		while(!currentLock.try_lock_for(std::chrono::milliseconds(300))){
//			if(mbResetActiveMapRequested){
////				mpLocalMapper->Release();
//				return;
