...
{
	mT_e0_ej=Eigen::Isometry3d::Identity();
	mT_g0_gj=Eigen::Isometry3d::Identity();
	mT_e_c=Eigen::Isometry3d::Identity();
	mT_e_g=Eigen::Isometry3d::Identity();
}
    ...
    mpLossRefKF(F.mpLossRefKF), mPoorVision(F.mPoorVision)
{
    ...
    for(int i=0; i<mnGridCols;i++)
    {
        ...
        for(int j=0; j<mnGridRows; j++){
			...
			assigned_feature_count+=F.mGrid[i][j].size();
            if(F.Nleft != -1){
                mGridRight[i][j] = F.mGridRight[i][j];
				assigned_feature_count+=F.mGridRight[i][j].size();
            }
        }
    }
//    cout<<"new keyframe, assigned feature: "<<assigned_feature_count<<endl;
...
cv::Mat KeyFrame::GetDvlRotation()
{
	unique_lock<mutex> lock(mMutexPose);
	// R_w_c * R_c_dvl
...
{
	unique_lock<mutex> lock(mMutexPose);
//	mRwc * mImuCalib.mT_c_dvl.rowRange(0, 3).col(3) + mOw;
	// R_c0_cj * cj_t_cj_dvlj + c0_t_c0_cj = c0_t_c0_dvlj
	return Twc.rowRange(0,3).colRange(0,3) * mImuCalib.mT_c_dvl.rowRange(0, 3).col(3) + Twc.rowRange(0,3).col(3);
}
...
vector<size_t> KeyFrame::GetFeaturesInArea(const float &x, const float &y, const float &r, const bool bRight) const
{
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        ...
        {
//        	cout<<"size of mGrid = ("<<mGrid.size()<<", "<<mGrid[ix].size()<<")"<<endl;
...
void KeyFrame::IntegrateDVL(KeyFrame *kp_i)
{
