...
public:
    ...
    void computeError()  {
        ...
        // xw: the pose of map point under world coordinate
        // v1->estimate().map(Xw): the pose of map point under camera coordinated
        // pCamera->project(v1->estimate().map(Xw)) : pixel coordinate of map point
...
public:
    ...
    void computeError()  {
        ...
        // xw: the pose of map point under world coordinate
        // v1->estimate().map(Xw): the pose of map point under camera coordinated
        // mTrl: transformation from left camera to right camera
...
// edge between 0: map point and 1: camera pose
...
// mTrl Transformation from left camera to right camera
// use this edge for those map points whose feature points only exist in right image.
...
public:
    ...
    bool isDepthPositive() {
        const g2o::VertexSE3Expmap* v1 = static_cast<const g2o::VertexSE3Expmap*>(_vertices[1]);
        const g2o::VertexSBAPointXYZ* v2 = static_cast<const g2o::VertexSBAPointXYZ*>(_vertices[0]);
        // convert coordinate from left camera to right camera
